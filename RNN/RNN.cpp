...
class FNN{
    public:
        ...
        void backward(Matrix input, Matrix output, Matrix context, Matrix context_prev, Matrix target){
            ...
            grad_c = delta_h2o * h2o.transposed() + delta_i2h*h2h.transposed();

            delta_i2h = grad_c & (temp_c - (c & c));
            
            grad_i2h = grad_i2h + x.transposed() * delta_i2h;
            grad_b_h = grad_b_h + delta_i2h;
            grad_h2h = grad_h2h + c_prev.transposed() * delta_i2h;
        }
